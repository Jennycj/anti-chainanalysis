...
function Form() {
  let [inputList, setInputList] = useState<Array<UtxoProps>>([
    { txid: "", vout: "" },
  ]);
  let [amountList, setAmountList] = useState<InfoProps>({
    amount: "",
    address: "",
  });
  let seperator = ":";
    ...
    if (result) {
      return (
        <div className="container">
          <div className="text">
            <p className="message">Message: {result}</p>
          </div>
          {result.length > 0 && (
