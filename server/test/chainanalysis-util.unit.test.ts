...
describe("antichainanalysis-util", () => {
  ...
  it("should sort utxos with respect to amount", () => {
    ...
    expect(
      chainAnalysisUtil.quickSort(utxos, 0, utxos.length - 1)[0].amountInSats
    ).to.equal(20);
    expect(
      chainAnalysisUtil.quickSort(utxos, 0, utxos.length - 1)[1].amountInSats
    ).to.equal(30);
    expect(
      chainAnalysisUtil.quickSort(utxos, 0, utxos.length - 1)[2].amountInSats
    ).to.equal(50);
  });
  ...
  it("should pass if transaction exist for address (address has been used before on signet)", () => {
    ...
    let response = chainAnalysisUtil.getTransactions(
      "tb1p0kvw0qkjtdv0qc02sd4q0xuq4kt0t3phg7xm7zqppljglgpaej0stzkn3s"
    );
    response.then((data) => {
  ...
  it("should pass if transaction does not exist for address (address has never been used before on signet)", () => {
    ...
    let response = chainAnalysisUtil.getTransactions(
      "tb1q30448v2wgrmtjsdemtlw5zdvneq0mjvg4d4m8g"
    );
    response.then((data) => {
      expect(data.length === 0).to.equal(true);
    });
